<template>
    <div class="container">
        ...
        <input v-model="claim.SN" v-on:keyup.enter="autoGenerate" class="form-control" type="text" id="serialnumber" placeholder="Serial Number">
        <button v-on:click="autoGenerate" class="btn btn-success mt-2">Auto-Generate</button>
        <form>
...
<script>
    export default {
        ...
        methods: {
            ...
            autoGenerate() {
                axios.get(`/api/order/${this.claim.SN}`)
                    .then(response => {
                        console.log(response.data)
                        let data = response.data['0']
                        this.claim.PO = data['Customer PO Number']
                        this.claim.PN = data['Part Number']
                        this.claim.Description = data['Description']
                        this.claim.Customer = data['Customer Name']
                        this.claim.Origin_WH = data['Warehouse']
                        axios.get(`/api/actprice/${this.claim.PN}`)
                            .then(response => {
                                console.log(response.data)
                                let data = response.data['0']
                                let cost = parseFloat(data['Actual Cost'])
                                this.claim.Actual_Cost = cost.toFixed(2).toString()
                            })
                            .catch(e => {
                                this.errors.push(e)
